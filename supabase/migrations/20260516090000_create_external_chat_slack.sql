...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'chat_sessions_source_check'
  ) then
    alter table public.chat_sessions
      add constraint chat_sessions_source_check
      check (source in ('web', 'slack', 'google_chat'));
  end if;

  if not exists (
    select 1 from pg_constraint where conname = 'chat_messages_source_check'
  ) then
    alter table public.chat_messages
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'agent_jobs_source_check'
  ) then
    alter table public.agent_jobs
      add constraint agent_jobs_source_check
      check (source in ('web', 'slack', 'google_chat'));
  end if;
end $$;
