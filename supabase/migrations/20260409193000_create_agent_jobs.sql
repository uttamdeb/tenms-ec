...
create policy if not exists "Users can view their own agent jobs"
  on public.agent_jobs
  for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own agent jobs"
  on public.agent_jobs
  for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own queued agent jobs"
  on public.agent_jobs
  for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);